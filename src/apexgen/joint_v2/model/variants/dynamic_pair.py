...
CONTRACT = dict(
    schema="apexgen.joint_v2.dynamic_pair.v1",
    features="Current pre-update rigid every iteration: 16 Gaussian RBF centers 0..30 A, width 2 A; local unit displacement, local displacement/(10 A + distance), flattened Ri^T Rj",
    mask="Valid non-self directed pairs with at least one peptide residue; pocket-pocket excluded",
    projection="Shared bias-free zero-initialized linear 31 -> pair_dim, added AFTER original static pair layer norm, no extra normalization",
    gradient="FP32 geometry construction, full recurrent rigid gradient; projection uses network autocast",
    objective="Unchanged original final denoise + clean-only sum of 8 proposal energies, coefficient 1",
    restoration="Old model/Adam/EMA/RNG preserved; appended new projection with fresh Adam group and zero EMA",
    scope="Exploratory native-sequence local geometry; no formal or full-base-generation claim",
)
CONTRACT_SHA256 = canonical_sha256(CONTRACT)
NEW_PREFIX = "decoder.structure_module.dynamic_pair."


def pair_features(rigid, residue_mask, peptide_mask, translation_scale):
    """SE(3)-invariant ordered features, using only current state and fixed pocket."""
    with torch.autocast(device_type=rigid.translation.device.type, enabled=False):
        x = rigid.translation.float() * translation_scale
        r = rigid.rotation.float()
        delta = x[:, None, :, :] - x[:, :, None, :]
        distance = torch.linalg.vector_norm(delta, dim=-1, keepdim=True)
        local = torch.einsum("bnji,bnmj->bnmi", r, delta)
        relative = torch.einsum("bnji,bmjk->bnmik", r, r).flatten(-2)
        centers = torch.linspace(0., 30., 16, device=x.device)
        rbf = torch.exp(-0.5 * ((distance - centers) / 2.).square())
        features = torch.cat((rbf, local / distance.clamp_min(1e-6), local / (10. + distance), relative), -1)
        valid = residue_mask[:, :, None] & residue_mask[:, None, :]
        valid = valid & (peptide_mask[:, :, None] | peptide_mask[:, None, :])
        valid = valid & ~torch.eye(x.shape[1], device=x.device, dtype=torch.bool)[None]
        return torch.where(valid[..., None], features, 0.)


class DynamicPairStructureModule(JointV2StructureModule):
    """Adopt the exact original modules; append one shared geometry projection.

    Forward is a frozen copy of the original with only pair injection and trace
    entries added. Keeping the historical module unchanged preserves audit hashes.
    """
    def __init__(self, base):
        ...
        self.dynamic_pair = nn.Linear(31, base.layer_norm_z.normalized_shape[0], bias=False)
        nn.init.zeros_(self.dynamic_pair.weight)
        ...
        *,
        ...
        for block_index in range(self.blocks):
            ...
            if attention_mask is None:
                single = single + self.ipa(conditioned, current_pair, rigid, condition.residue_mask)
            else:
                single = single + self.ipa(
                    conditioned, current_pair, rigid, condition.residue_mask, attention_mask=attention_mask
                )
